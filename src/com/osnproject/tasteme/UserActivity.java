...
public class UserActivity extends Activity{
	...
	public Group getRecommend(List<Group> groups){           //TODO: the algorithm to get the recommended group
		int a,i,j,n,m,p,q,d;
		int groupresult=0;
		int groupproductId[] = new int [15];
		int userproductId[]= new int [15];
		int usernum[]=new int[15];
		int score[] = new int [15];
	
		for(a=0;a<15;a++){
			score[a]=0;
		}
		for(p=0;p<grouplikes.size();p++){             //get p groups
		    GroupLike grouplike = grouplikes.get(p);
		    
		    for(i=0;i<grouplike.productId.size();i++){   //get NO.p group's productId and usernumber and put them into array
		       groupproductId[i] =grouplike.productId.get(i);
		       usernum[i] = grouplike.userNum.get(i);
		       
		    		   }
		    for (q=0;q<mylikes.size();q++){
		    	ActionLike mylike = mylikes.get(q);
		    	userproductId[q]=mylike.getProductId();
		    }
		    for(m=0;m<groupproductId.length;m++){
		    	for(n=0;n<userproductId.length;n++){
		    		if(groupproductId[m]==userproductId[n]){
		    			score[p]=score[p]+usernum[m];
		    		}
		    	}
		    }
		...
		for(d=0;d<15;d++){
			if(score[d]>temp){
				temp=score[d];
		        groupresult=grouplikes.get(d).groupId;
			}
		}
		
		if(groupresult == 0)
			return findGroupById(6);
		else
			return findGroupById(groupresult);
	}
	...
	public AdapterView.OnItemClickListener groupClicker = new AdapterView.OnItemClickListener() {            //a group is selected
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			AlertDialog.Builder createDialog=new AlertDialog.Builder(UserActivity.this);  //create the dialog
	...
	public class GetUserThread extends Thread{
		...
		public void run(){
			...
			case MSG_GET_GROUPS:
				...
				for(int i=0; i<groups.size(); i++){
					int groupid = groups.get(i).getGroupId();                        //get current group
					List<ActionLike> likes = rc.getLikeByGroup(groupid);           //get the likes of the group
					...
					GroupLike grouplike = new GroupLike();          //create a new group like
					
					grouplike.groupId = groupid;
					
					for(int j=0;j<likes.size();j++){
						ActionLike like = likes.get(j);            //traverse every like
						
						if(grouplike.productId.isEmpty()){        //if current groupLike has no element, create one
							int count = 1;
						...
						if(like.getProductId() < grouplike.productId.get(grouplike.productId.size()-1))   //this won't happen
							continue;
						else if(like.getProductId() == grouplike.productId.get(grouplike.productId.size()-1)){   //if there is one, add its count
							int count = grouplike.userNum.get(grouplike.productId.size()-1);
							count ++;
							grouplike.userNum.set(grouplike.productId.size()-1, count);
						}else{                      //if not there, create a new one
							int count = 1;
							int productid = like.getProductId();
							grouplike.productId.add(productid);
							grouplike.userNum.add(count);
